---
name: ChinapostAMC_FactsheetUpdate-ValueOne
description: 价值1号策略实盘汇报一键更新 — 自动拉取估值表/聚源指数/行业分类/市场风格数据，更新第2页8个数据区域。触发词:/update-value1、更新价值1号实盘报告
---

# /ChinapostAMC_FactsheetUpdate-ValueOne — 价值1号策略实盘汇报更新

## 角色定位

为"中邮资管价值1号量化选股策略资产管理产品"的实盘汇报 HTML 执行一键数据更新。

- 第1页（策略介绍 + 回测绩效）：**固定不动，永不修改**
- 第2页（实盘运行情况）：**8 个 AUTO-BEGIN/END 标记区域自动替换**

运行 `update_factsheet.py` 脚本，自动从估值表 Excel (.xls)、聚源 DuckDB、东方财富 API 拉取最新数据。

## 路由

无 MCP Server 依赖。使用 Bash 工具执行 Python 脚本。依赖本地环境：Python 3.13、xlrd、duckdb、numpy。
...
| 数据 | 路径 |
|------|------|
| 估值表 | `/Users/junye_shi/中邮资管/中邮金市/target_list/产品净值/中邮价值1号/` |
...
| HTML 模板 | `/Users/junye_shi/中邮资管/中邮金市/2026/价值1号路演材料/价值1号策略_实盘汇报.html` |
| 脚本 | `/Users/junye_shi/中邮资管/工作文件依赖项/中邮产品实盘报告/ValueOne/update_factsheet.py` |
...
1. **命名格式**：`【产品名称】回测及实盘情况汇报【生成日期】.html`
   - 示例：`【中邮价值1号】回测及实盘情况汇报【2026-07-19】.html`
2. **输出位置**：`/Users/junye_shi/中邮资管/中邮金市/2026/价值1号路演材料/`
3. **归档规则**：写入新报告前，自动将路演材料目录下所有旧 `【*】回测及实盘情况汇报【*】.html` 移入 `往期报告/` 子目录
4. **模板保护**：`价值1号策略_实盘汇报.html`（含 AUTO 标记的模板）不会被归档，每次从模板读取后写入新文件
...
```bash
python3 /Users/junye_shi/中邮资管/工作文件依赖项/中邮产品实盘报告/ValueOne/update_factsheet.py --dry-run
```
...
```bash
python3 /Users/junye_shi/中邮资管/工作文件依赖项/中邮产品实盘报告/ValueOne/update_factsheet.py
```
...
| 上游/下游 | Skill | 数据流 |
|-----------|-------|--------|
| 同级 | `ChinapostAMC_FactsheetUpdate-DividendQuality` | 同模块，脚本结构一致，独立运行 |
